...
def test_health():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"   ✅ Backend online: {data.get('service')}")
            return True
        else:
        ...
        print(f"   💡 Start backend with: python backend/main.py")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                print(f"   ✅ Write access: {data.get('has_write_access')}")
                return True
            else:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                if dispatch.get("success"):
                    print(f"   ✅ repository_dispatch triggered")
                    print(f"      Event: {dispatch.get('event_type')}")
                    print(f"      Payload: {json.dumps(dispatch.get('payload'), indent=8)}")
                    return True
                else:
                    print(f"   ⚠️  Dispatch failed: {dispatch.get('error')}")
                    return True  # Commit succeeded even if dispatch failed
            else:
                print(f"   ❌ Commit failed: {data.get('error')}")
                return False
        else:
            error = response.json().get("detail", "Unknown error")
            print(f"   ❌ Commit request failed: {error}")
            return False
